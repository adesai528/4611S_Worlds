...
float lastYaxo = 0.0; // Track the previous state of the horizontal pod

// Constants
const float DRIVE_WHEEL_DIAMETER = 3.25; // Drivetrain wheels
const float ODOM_WHEEL_DIAMETER = 2.0;  // yaxo odom wheel
const float TICKS_PER_REV = 360.0;

void updateOdometry(){ //Run as a thread
    while(true) {
        // 1. Get current heading in radians
        heading = inert.heading();
        float theta = heading * M_PI / 180.0;

        // 2. Get drivetrain encoder position (Forward/Backward)
        float currentEncoder = (LeftMotorGroup.position(degrees) + RightMotorGroup.position(degrees)) / 2.0;
        float deltaForwardTicks = currentEncoder - lastEncoder;
        lastEncoder = currentEncoder;

        // 3. Get yaxo pod position (Left/Right)
        float currentYaxo = yaxo.position(degrees);
        float deltaSideTicks = currentYaxo - lastYaxo;
        lastYaxo = currentYaxo;

        // 4. Convert ticks to distance using specific diameters
        float distForward = (deltaForwardTicks / TICKS_PER_REV) * (M_PI * DRIVE_WHEEL_DIAMETER);
        float distSideways = (deltaSideTicks / TICKS_PER_REV) * (M_PI * ODOM_WHEEL_DIAMETER);

        // 5. Update global x, y using a rotation matrix to account for heading
        XPosition += (distForward * cos(theta)) - (distSideways * sin(theta));
        YPosition += (distForward * sin(theta)) + (distSideways * cos(theta));

        wait(20, msec);  // 50 Hz update rate
    }
}
